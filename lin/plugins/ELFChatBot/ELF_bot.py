...
async def handle_first_receive(bot: Bot, event: Event, state: dict):
    ...
    try:
        ...
        if API_Key and Secret_Key and bot_id:
            Bot = baiduBot.BaiduBot(API_Key=API_Key, Secret_Key=Secret_Key, bot_id=bot_id, session=session)
        else:
            logger.error('百度闲聊配置出错！将使用腾讯闲聊！')
            if app_id and appkey:
                Bot = txbot.TXBot(app_id=app_id, appkey=appkey, session=session)
            else:
    ...
    msg = remove_cqcode(str(event.message))
    r_msg = await Bot.sendMsg(msg)
    r_msg = r_msg['answer'].replace('王二小', '凌').replace('张得亮', '凌')
    await ELF_bot.finish(MessageSegment.text(r_msg))
