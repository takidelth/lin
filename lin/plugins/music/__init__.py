...
async def _handle_event(bot: Bot, event: MessageEvent, state: T_State) -> None:
    if not "music.163.com" in (link_str := state["link_str"]):
        await music.reject("目前只支持网易云音乐喔")
    
    MusicParser = MusicParse()
    MusicParser.link = link_str
    if MusicParser.is_ok():
    ...
    if MusicParser.get_type == "song" and content:
        cover = MessageSegment.image(content['cover'])
        repo = cover + Message(f"\n歌名：{content['name']}\n"
            f"歌手：{content['artist']}\n"
    ...
    elif MusicParser.get_type == "playlist":
